...
// حارس الأخطاء لمنع الشاشة البيضاء
class ErrorBoundary extends Component<{children: ReactNode}, {hasError: boolean}> {
  ...
  render() {
    if (this.state.hasError) return <div style={{padding: '50px', textAlign: 'center', color: 'red'}}>Erreur de chargement de l'interface. Veuillez vérifier la console (F12).</div>;
  ...
  return (
    <div style={{ background: "#F5F7FA", minHeight: "100vh", fontFamily: "Inter, sans-serif", direction: isRTL ? "rtl" : "ltr" }}>
      <header style={{ background: "linear-gradient(135deg, #1F4E79 0%, #2E86DE 100%)", color: "#fff", padding: "12px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 2px 12px rgba(0,0,0,0.18)" }}>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(255,255,255,0.15)", padding: "6px 12px", borderRadius: "8px" }}>
            ...
            <input value={deviceId} onChange={e => setDeviceId(e.target.value)} style={{ background: "transparent", border: "none", color: "#fff", fontSize: "13px", fontWeight: "bold", width: "72px", outline: "none" }} />
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(255,255,255,0.15)", padding: "6px 12px", borderRadius: "8px" }}>
            <span style={{ fontSize: "11px", opacity: 0.8 }}>🗂 {t("dashboard.project")}:</span>
            <input value={projectId} onChange={e => setProjectId(e.target.value)} style={{ background: "transparent", border: "none", color: "#fff", fontSize: "13px", fontWeight: "bold", width: "72px", outline: "none" }} />
          </div>
          <LanguageSwitcher />
          ...
          <button onClick={() => setUser(null)} title={t("nav.logout")} style={{ background: "rgba(231,76,60,0.25)", border: "1.5px solid rgba(231,76,60,0.5)", color: "#fff", padding: "6px 12px", borderRadius: "8px", cursor: "pointer", fontSize: "13px" }}>🚪</button>
        </div>
      </header>
      <nav style={{ background: "#fff", padding: "0 28px", display: "flex", gap: "4px", borderBottom: "2px solid #E5E7EB", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "13px 20px", border: "none", background: "transparent", color: activeTab === tab.id ? "#1F4E79" : "#6B7280", fontWeight: activeTab === tab.id ? "700" : "400", borderBottom: activeTab === tab.id ? "3px solid #1F4E79" : "3px solid transparent", cursor: "pointer", fontSize: "14px", transition: "all 0.15s", marginBottom: "-2px", whiteSpace: "nowrap" }}>{tab.label}</button>
        ))}
      </nav>
